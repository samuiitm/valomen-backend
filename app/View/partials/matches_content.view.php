...
if ($searchMatches !== '') {
    $scheduleParams['search'] = $searchMatches;
    $resultsParams['search']  = $searchMatches;
}

$scheduleUrl = url('matches') . '?' . http_build_query($scheduleParams);
$resultsUrl  = url('matches') . '?' . http_build_query($resultsParams);
?>
            ...
            <?php if (!$boolSchedule): ?>
                <div class="matches-header-right">
                    <div class="matches-tabs">
                        <a href="<?= htmlspecialchars($scheduleUrl) ?>"
                           class="tab js-matches-ajax-link <?= $view === 'schedule' ? 'active' : '' ?>">
                            SCHEDULE
                        </a>

                        <a href="<?= htmlspecialchars($resultsUrl) ?>"
                           class="tab js-matches-ajax-link <?= $view === 'results' ? 'active' : '' ?>">
                            RESULTS
                        </a>
                    </div>

                    <form class="matches-search" id="matchesSearchForm" action="<?= url('matches') ?>" method="get">
                    ...
                    <?php if (!empty($_SESSION['is_admin']) && !empty($_SESSION['edit_mode'])): ?>
                        <a href="<?= url('match_create') ?>" class="add-match-btn">
                ...
                <div class="matches-day">
                    <?php foreach ($matchesOfDay as $match): ?>
                        ...
                        <div class="match-block">
                            ...
                            <?php elseif ($hasPred): ?>
                                <a href="<?= url('predict') ?>?match_id=<?= (int) $match['id'] ?>"
                                   class="predict-button">
                                    Edit prediction
                                </a>

                            <?php else: ?>
                                <a href="<?= !empty($_SESSION['user_id']) ? url('predict') . '?match_id=' . (int) $match['id'] : url('login') ?>"
                                   class="predict-button">
                                    Make prediction
                                </a>
                            <?php endif; ?>
                            ...
                            <div class="tournament-info">
                                ...
                                <img class="tournament-logo"
                                     src="<?= url('assets/icons/events/' . rawurlencode($match['event_logo'])) ?>"
                                     alt="Tournament-icon">
                            </div>

                            <?php if (!empty($_SESSION['is_admin']) && !empty($_SESSION['edit_mode'])): ?>
                                <div class="match-admin-actions">
                                    <a href="<?= url('match_edit') ?>?id=<?= (int) $match['id'] ?>"
                                       class="match-admin-btn edit"
                                       title="Edit match">
                                        ✎
                                    </a>

                                    <a href="<?= url('match_delete') ?>?id=<?= (int) $match['id'] ?>&view=schedule"
                                       class="match-admin-btn delete js-delete-match"
                                       data-match-label="<?= htmlspecialchars(($match['team_1_name'] ?? 'TBD') . ' vs ' . ($match['team_2_name'] ?? 'TBD')) ?>"
                                       title="Delete match">
                                        🗑
                                    </a>
                                </div>
                        ...
                        <div class="match-block">
                            ...
                            <?php if (!empty($_SESSION['is_admin']) && !empty($_SESSION['edit_mode'])): ?>
                                <div class="match-admin-actions">
                                    ...
                                    <a href="<?= url('match_delete') ?>?id=<?= (int) $match['id'] ?>&view=results"
                                       class="match-admin-btn delete js-delete-match"
                                       data-match-label="<?= htmlspecialchars(($match['team_1_name'] ?? 'TBD') . ' vs ' . ($match['team_2_name'] ?? 'TBD')) ?>"
                                       title="Delete match">
                                        🗑
...
<nav class="pager">
    <form class="filters-pag" id="matchesFiltersForm" action="<?= url('matches') ?>" method="get">
        <input type="hidden" name="view" value="<?= htmlspecialchars($view) ?>">
        <input type="hidden" name="search" value="<?= htmlspecialchars($searchMatches) ?>">
        <input type="hidden" name="p" value="1">
    ...
    <?php if ($totalPagesMb > 1 && $searchMatches === ''): ?>
        <nav class="pager">
            <a href="<?= build_matches_url(1, $perPage, $view, $orderMatches, $searchMatches) ?>"
               class="btn js-matches-ajax-link<?= $currentPage === 1 ? ' is-disabled' : '' ?>">
                « First
            </a>
            ...
            <?php for ($p = $startPage; $p <= $endPage; $p++): ?>
                ...
                <?php else: ?>
                    <a href="<?= build_matches_url($p, $perPage, $view, $orderMatches, $searchMatches) ?>"
                       class="page js-matches-ajax-link">
            ...
            <a href="<?= build_matches_url(min($totalPagesMb, $currentPage + 1), $perPage, $view, $orderMatches, $searchMatches) ?>"
               class="btn js-matches-ajax-link<?= $currentPage === $totalPagesMb ? ' is-disabled' : '' ?>">
                Next ›
            </a>
